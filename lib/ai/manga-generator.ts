...
type PageBreakdownEntry = StoryStructure["pageBreakdown"][number];

function createFallbackPanels(pageBreakdown: PageBreakdownEntry): MangaPanel[] {
  return [
    {
      panelNumber: 1,
      description: pageBreakdown.summary,
...
function pushFallbackPage(
  pagesData: MangaPage[],
  pageBreakdown: PageBreakdownEntry
) {
  pagesData.push({
    pageNumber: pageBreakdown.pageNumber,
    panels: createFallbackPanels(pageBreakdown),
  });
}

function extractPanelJsonObject(rawText: string): string | null {
  if (!rawText) {
    return null;
  }
  ...
  if (!jsonText.startsWith("{")) {
    ...
    jsonText = jsonText.substring(firstBrace).trim();
  }
...
Return ONLY the JSON object, nothing else.`,
      ...
      if (!result || !result.content) {
        pushFallbackPage(pagesData, pageBreakdown);
        continue;
      }
      ...
      if (
        ...
        console.warn(
          `LLM returned error for page ${pageBreakdown.pageNumber}:`,
      ...
      const normalizedJson = extractPanelJsonObject(jsonText);
      if (!normalizedJson) {
        console.warn(
          `Could not extract JSON for page ${pageBreakdown.pageNumber}, using fallback panels`
        );
        pushFallbackPage(pagesData, pageBreakdown);
        continue;
      }

      jsonText = normalizedJson;
      ...
      try {
        ...
        console.error(
          `JSON parse error for page ${pageBreakdown.pageNumber}:`,
          parseError
        );
      ...
      pagesData.push({
        ...
        panels: parsed.panels.map((p: any, idx: number) => ({
          panelNumber: typeof p.panelNumber === "number" ? p.panelNumber : idx + 1,
          description: typeof p.description === "string" ? p.description : pageBreakdown.summary,
          dialogue: Array.isArray(p.dialogue) ? p.dialogue : [],
          visualStyle: typeof p.visualStyle === "string" ? p.visualStyle : "standard",
        })),
  ...
  if (pagesData.length < storyStructure.pageBreakdown.length) {
    console.warn(
      `Only generated ${pagesData.length}/${storyStructure.pageBreakdown.length} pages, filling remaining with fallbacks`
    );
    for (
      let i = pagesData.length;
      i < storyStructure.pageBreakdown.length;
      i++
    ) {
